    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
        ...
        _data.read('tokens', token, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, usereData) => {
                    if (!err && usereData) {

                        const userChecks = typeof usereData.checks == 'object' && usereData.checks instanceof Array ? usereData.checks : [];
                        if (userChecks.length < config.maxChecks) {
                            ...
                            _data.create('checks', checkId, checkObject, (err) => {
                                if (!err) {
                                    ...
                                    usereData.checks = userChecks;
                                    usereData.checks.push(checkId);
...
handlers._checks.get = (data, callback) => {
    ...
    if (id) {
        ...
        callback(400, {'Error' : 'Nedo0staje id'});
    }
};
